...
public class SortRunner {
	...
	private static void runAllSortsForOneArray(int[] array) {
		...
		startTime = System.currentTimeMillis();
		Set<Integer> sortedTree = new TreeSet<Integer>();
		for(int i : sortedIntegersUsingDefaultSort){
			sortedTree.add(i);
		}
		elapsedTime = (System.currentTimeMillis() - startTime);
		isSorted = verifySort(sortedIntegersUsingDefaultSort);
		displayResults("Integer", "treeSort", elapsedTime, size, isSorted);
		
		// TODO: your implementation of quick sort. I suggest putting this in a static method in a Quicksort class.
		
		// MAYBEDONE: your BinaryHeap sort. You can put this sort in a static method in another class. 
		startTime = System.currentTimeMillis();
		BinaryHeap sortedHeap = new BinaryHeap<Integer>();
		sortedHeap.sort(sortedIntegersUsingHeapSort);
		elapsedTime = (System.currentTimeMillis() - startTime);
	...
	private static int[] getUniqueElementArray(int size) {
		...
		ArrayList<Integer> toConvert = new ArrayList<>();
		for(int i=0; i<size; i++){
			toConvert.add(i);
		}
		Collections.shuffle(toConvert);
//		retArray = toConvert.toArray(Integer);
//		retArray = toConvert.toArray();
		return retArray;
	}
