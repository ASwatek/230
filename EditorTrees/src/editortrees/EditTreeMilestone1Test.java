...
//		assertEquals("abc", t.toString());
	...
	@Test
	public void selfCheckAddOrderAndCompareLeftSizeToRank(){
		/**
		 * structures & amount of things
		 */
		int testNum = 800;
		EditTree t = new EditTree();
		ArrayList<Character> ret = new ArrayList<Character>();
		char[] nRet = new char[testNum];
		
		//parallel arrays to compare root rank and left size
		int[] parAr1 = new int[testNum];
		int[] parAr2 = new int[testNum];
		
		char[] comAr1 = new char[testNum];
		char[] comAr2 = new char[testNum];

		
		/**
		 * makes random characters to add to structures
		 */
	    final String alphabet = "0123456789ABCDE";
	    final int N = alphabet.length();
	    Random r = new Random();
	    for (int i = 0; i < testNum; i++) {
	    	char toAdd = alphabet.charAt(r.nextInt(N));
	    	ret.add(toAdd);
	        t.add(toAdd);
	        nRet[i]=toAdd;
	        parAr1[i] = t.getRoot().left.size();
//	        parAr2[i] = t.getRank();
	    }
	    
		//needs to recurse to check them all
		for(int i=0; i<t.height(); i++){
			if(t.getRoot().left.height()==t.getRoot().right.height()) comAr1[i]='=';
			if(t.getRoot().left.height()>t.getRoot().right.height()) comAr1[i]='/';
			if(t.getRoot().left.height()<t.getRoot().right.height()) comAr1[i]='\\';
			
			comAr2[i] = t.getRoot().balance.toString().charAt(0);
		}
	    
	    /**
	     * checks if order added is the same
	     */
		for(int i=0; i<testNum; i++){
			System.out.print(nRet[i]+ ", ");
		}
		System.out.println("\nbreak");
		for(int i=0; i<testNum; i++){
		...
		System.out.println("\nbreak");
		/**
		 * checks if rank and size are the same
		 */
		for(int i=0; i<testNum; i++){
			System.out.print(parAr1[i] + ", ");
		}
		System.out.println("\nbreak");
		for(int i=0; i<testNum; i++){
			System.out.print(parAr2[i] + ", ");
		}
		System.out.println("\nbreak");
		for(int i=0; i<testNum; i++){
			System.out.print(comAr1[i]);
		}
		System.out.println("\nbreak");
		for(int i=0; i<testNum; i++){
			System.out.print(comAr2[i]);
		}
		
		/**
		 * null comparison to easily find
		 */
		String usedToFail = "a";
		assertEquals("2", usedToFail.charAt(14));
	}
